---
name: exampass
description: 将课程资料（PPT/Word/PDF）按章节生成知识清单和交互式章节测试，帮助高效期末复习。触发条件：用户提到或操作来自 C:\Users\MR\Desktop\教育教学 的文件时自动调用。
---
...
- **公式可视化（6 模式）**：核心公式不得只写一行 LaTeX，必须选配合适的可视化 pattern（详见表后说明）

### 公式可视化 · 10 种模式

遇到公式必须图文结合。根据公式类型选择其一（最多选 1-2 个，非全部都要）：

**基础 6 模式**（适用单个公式/推导）：

| # | 模式 | CSS class | 适用场景 | 结构 |
|---|------|-----------|---------|------|
| 1 | 公式解剖卡 | `fv-anatomy` | 首次出现的核心公式 | 上半公式 + 下半多列符号卡片（名称/直觉/作用），每列不同颜色 `.fv-c{N}` |
| 2 | 推导链路图 | `fv-derive-chain` | 多步变换（损失/梯度链） | 用 → 串联公式盒，下标注"做什么"和"为什么" |
| 3 | 视觉类比图 | `fv-analogy` | 有物理直觉的概念（卷积/Dropout） | 左 SVG 示意图 + 右公式说明 |
| 4 | 折叠逐步推导 | `fv-derive-steps` | ≥4 步推导 | 每行：编号圆圈 + 公式盒 + 解释，`<details open>` |
| 5 | 参数快查表 | `fv-params` | ≥4 个超参数的公式（Adam/BatchNorm） | 列：符号 / 含义 / 典型值 / 作用，符号列用 `fv-param-sym` |
| 6 | 函数图像 | `fv-plot` | 激活函数、损失函数等可画曲线 | 手写 SVG（坐标轴+曲线+标注），底部横线说明 |

**增强 4 模式**（适用需要对比/多维/诊断的场景）：

| # | 模式 | CSS class | 适用场景 | 结构 |
|---|------|-----------|---------|------|
| 7 | 3D 概念图 | `fv-3d-concept` | SVM 超平面、损失曲面等空间概念 | SVG 透视投影（3D坐标框架+半透明面+标注）；顶部配 `.fv-legend-bar` 图例 |
| 8 | 多维诊断面板 | `fv-multiview` | 损失曲面双视角、混淆矩阵+指标卡 | `.fv-mv-row` 放多个 `.fv-mv-panel`，可左等高线右3D曲面 |
| 9 | 联合公式解剖 | `fv-anatomy-plus` | ≥2 个公式串联（Softmax→CE） | 每公式一个 `.fv-ap-panel`，`.fv-ap-connector` 标连接关系 |
| 10 | 多曲线对比图 | `fv-plot-enhanced` | ≥3 条激活/损失曲线对比 | 顶部 `.fv-legend-bar`（`.fv-leg` + `.fv-leg-swatch`），双Y轴标注 |

**图例通用写法**（所有多线/多区图表公用）：
```html
<div class="fv-legend-bar">
  <div class="fv-leg"><span class="fv-leg-swatch" style="background:#2563eb"></span> 曲线A含义</div>
  <div class="fv-leg"><span class="fv-leg-swatch sw-line" style="background:#dc2626"></span> 虚线含义</div>
  <div class="fv-leg"><span class="fv-leg-swatch" style="background:#fbbf24;border-radius:50%;width:12px;height:12px"></span> 点含义</div>
</div>
```

CSS 定义于 `templates/formula_viz.css`（知识页和题库页均加载）；子 Agent prompt 细则见 `agents/notes-agent.md` Part 4。
